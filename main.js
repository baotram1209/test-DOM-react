...
//UI components - Ko xử lý logic trong này
function CourseItem({ data, onClick }) {
  return (
    <div className="course-item">
      <img src={data.image_url} alt={data.title} />
      <h2 className="course-title" onClick={() => onClick(data)}>
        {data.title}
      </h2>
...
//Container
function App() {
  //Đặt tên function : thay on bằng handle
  //useCallback
  ...
  return (
    <div id="wrapper">
      {courses.map((course) => (
        <CourseItem key={course.id} data={course} onClick={handleClick} />
      ))}
    </div>
  );
}
